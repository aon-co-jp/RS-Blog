...
impl SmtpConfig {
    pub fn from_env() -> Option<Self> {
        Some(Self {
            host: std::env::var("RSBLOG_SMTP_HOST").ok()?,
            port: std::env::var("RSBLOG_SMTP_PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(587),
            username: std::env::var("RSBLOG_SMTP_USERNAME").ok()?,
            password: std::env::var("RSBLOG_SMTP_PASSWORD").ok()?,
            from: std::env::var("RSBLOG_SMTP_FROM").ok()?,
        })
    }
}
...
pub async fn send_otp(config: SmtpConfig, to: String, code: String) -> Result<(), MailError> {
    let body = format!(
        "RS-Blog ログイン用ワンタイムパスワード\n\n\
         コード: {code}\n\
         このコードは5分間有効です。\n\n\
         心当たりがない場合はこのメールを無視してください。"
    );
    tokio::task::spawn_blocking(move || build_and_send(&config, &to, "RS-Blog ログインコード", body))
        .await
